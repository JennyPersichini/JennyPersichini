...
function getTotalAmount() {
    ...
    for(let i = 0; i < productsInCart.length; i++) {
        totale += productsInCart[i].price * productsInCart[i].quantity;
    }
    return totale;
}
...
function getCategoryCode(categoriesName) {
   let item = categories.find(item => item.name == categoriesName);
   return item.id ; 
}
...
function getCategoryCount(categoriesName) {
    productsInCart.filter(prodotto => prodotto.category === getCategoryCode(categoriesName)).length
}
...
*/
 ...
 function removeFromCart(id) {
    const prodotto = productsInCart.find(prodotto => prodotto.id === id)
    prodotto.quantity -= 1

    if(prodotto.quantity = 0) {
        productsInCart.splice(prodotto, 1)
...
*/
 ...
 function getPizzeBianche() {
    productsInCart.filter(prodotto => prodotto.category === 0 && !prodotto.ingredients.include('pomodoro'))
}
